    ...
    private void createAndConnectEdges() {
        ...
        for (Line line : lines) {
            ...
            for (int i = 0; i < stops.size() - 1; i++) {
                ...
                int fromTime = times.get(i);
                int toTime = times.get(i + 1);
                double distance = fromStop.distanceTo(toStop);
                Edge forwardEdge = new Edge(fromStop, toStop, line.getType(), line, toTime - fromTime, distance);
                fromStop.addForwardEdge(forwardEdge);
                Edge backwardEdge = new Edge(toStop, fromStop, line.getType(), line, toTime - fromTime, distance);
                toStop.addBackwardEdge(backwardEdge);
            }
        }
    }
